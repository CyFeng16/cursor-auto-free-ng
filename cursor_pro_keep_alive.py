...
def resource_path(relative_path):
    ...
    try:
        ...
        # 如果不是通过 PyInstaller 运行，则使用当前文件所在的目录
        base_path = os.path.dirname(os.path.abspath(__file__)) # 或者 os.path.abspath(".") 如果希望相对于脚本启动位置
...
class EmailGenerator:
    ...
    def load_names(self):
        # 使用 resource_path 函数获取 names-dataset.txt 的正确路径
        file_path = resource_path("names-dataset.txt")
        try:
            with open(file_path, "r") as file:
                return file.read().split()
        except FileNotFoundError:
            logging.error(f"错误：无法找到必要的数据文件: {file_path}")
            # 可以选择抛出异常或返回空列表等处理方式
            raise FileNotFoundError(f"无法找到 names-dataset.txt 在路径: {file_path}")
